...
type TourHeroProps = {
  tourHeroData: CountryContentProp
}

function TourHero({tourHeroData }: TourHeroProps) {
  return (
    <div className='w-full py-12 bg-gradient-to-br from-gray-50 to-gray-100'>
        <section className='max-container w-[95%] mx-auto flex flex-col gap-8'>
            ...
            <div className='lg:w-[95%] w-full mx-auto text-justify md:py-8 py-5 flex flex-col md:gap-4 gap-3'>
                ...
                >
                    {tourHeroData?.history?.title}
                </Typography>
                <div className='flex flex-col md:gap-4 gap-2'>
                    {tourHeroData?.history?.contents.map(data => (
                        <Typography
                            key={data}
